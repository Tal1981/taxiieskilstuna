...
const Card = (children: { destination: string, prise: string, old_prise: string, delay: string, darkMode: boolean }) :JSX.Element => {
    ...
    return (
        <div className={`w-full max-w-sm p-3 border-2 border-orange-300 rounded-lg shadow sm:p-8 transition duration-[5000ms] ease-in-out ${children.darkMode? 'bg-[darkColor]':'white'} ${parallex? 'translate-x-0 opacity-100':'translate-x-[-400px] opacity-0'}`} style={{transitionDelay:`${children.delay}ms`}} >
